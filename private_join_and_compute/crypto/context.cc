...
// 加入sm3
std::string Context::Sm3String(absl::string_view bytes) {
  unsigned char hash[EVP_MAX_MD_SIZE];
  CRYPTO_CHECK(1 ==
               EVP_DigestInit_ex(evp_md_ctx_.get(), EVP_sm3(), nullptr));
  CRYPTO_CHECK(
...
BigNum Context::RandomOracle(absl::string_view x, const BigNum& max_value,
  ...
  for (int i = 1; i < iter_count + 1; i++) {
    ...
    if (hash_type == SHA512) {
      ...
      // 加入sm3
      hashed_string = Sm3String(bignum_bytes);
    } else {
...
BigNum Context::RandomOracleSm3(absl::string_view x,
                                   const BigNum& max_value) {
  // 加入sm3
  return RandomOracle(x, max_value, SM3);
}
