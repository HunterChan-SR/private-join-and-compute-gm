...
StatusOr<ECPoint> ECGroup::GetPointByHashingToCurveSm3(
    absl::string_view m) const {
  BigNum x = context_->RandomOracleSm3(m, curve_params_.p);
  while (true) {
    auto status_or_point = GetPointByHashingToCurveInternal(x);
    if (status_or_point.ok()) {
      return status_or_point;
    }
    x = context_->RandomOracleSm3(x.ToBytes(), curve_params_.p);
  }
}
